...
At least for now, this repository only contains the Rust version parts of my code (with Assembly inline). The C language source code is currently "trapped" inside the university network drive, to which I have no free access from home, and after the course ended I have not had much business to university computers. Maybe I can some day salvage that too.

I'm not sure if the C project template exists anywhere in public. The original Rust project template can be found here: https://github.com/hegza/alien-shooter-template-rs
